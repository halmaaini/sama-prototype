...
| Term | Meaning |
|------|---------|
| **Activity** | Any planned offering in SAMA: event, program, volunteering opportunity, internal task, or externally-reported activity. |
| **Event** | Single or multi-day activity where students register and attend. Covers lectures, ceremonies, competitions, cultural events, sports matches, conferences, health awareness sessions, etc. |
| **Program** | Structured multi-session activity with an attendance threshold for completion (e.g. 80%). Covers training series, bootcamps, lecture series, sports seasons. |
| **Volunteering** | Activity where students sign up to give time or effort. Hours are tracked toward the semester volunteer target. Covers CSR drives, charity work, community service, blood donation. |
| **Task** | Internal staff coordination item. Not student-facing; no registration. Covers ops, admin, and planning activities. |
| **External** | Activity taking place outside UD that students self-report or are pre-registered for. Covers external competitions, off-campus conferences, external volunteering. Enters a pending-verification state before counting toward the student's record. |
| **Activity category** | A thematic classification applied to every activity at creation. One of 12 system defaults (fully editable and disableable by Manager from Settings): Sports · Social & Entertainment · Awareness / Education · Community Service · Operations & Admin · Awards & Recognition · Cultural · Training & Development · Academic & Professional Competitions · Conference / Exhibition · Health & Wellness · Orientation & Induction. |
| **Activity tag** | An optional free-text label applied to an activity for granular filtering and reporting. Tags are open and reusable across all categories and types. Any coordinator may add a new tag on the fly during activity creation; Manager curates the tag library from Settings. |
...
- **Event**: single or multi-day; students register and attend; attendance recorded once (or once per day for multi-day); certificate issued if attendance threshold met (default: present). Covers all single-instance activities regardless of category.
- **Program**: multi-session with a defined schedule; attendance recorded per session; completion when `attended_sessions / total_sessions ≥ completion_threshold` (default 80%); certificate on completion.
- **Volunteering**: students sign up for a slot or a shift; hours logged on check-in/check-out or manually by coordinator; hours count toward semester volunteer target; certificate optional (configurable per activity).
- **Task**: staff-only; no student registration, no student-facing catalog entry, no certificate. Used for internal coordination, planning items, and admin workflows.
- **External**: student self-reports participation in an activity outside UD (or coordinator pre-registers them); enters `pending_verification` state; coordinator reviews and confirms; on confirmation hours/completion count toward student record. Certificate not issued by SAMA (external cert upload optional).
...
### 6.1 Create & edit
- Coordinator opens "New Activity", picks type and category, fills fields. The form adapts per type — fields are shown, hidden, or marked required based on the matrix below.

#### 6.1.1 Field matrix by type

`✅ Required` · `○ Optional` · `— Hidden / not applicable`

| Field | Event | Program | Volunteering | Task | External |
|---|:---:|:---:|:---:|:---:|:---:|
| Title | ✅ | ✅ | ✅ | ✅ | ✅ |
| Category | ✅ | ✅ | ✅ | ✅ | ✅ |
| Tags | ○ | ○ | ○ | ○ | ○ |
...
| End date/time | ○ | ○ | ○ | ○ | ○ |
| Location | ✅ | ✅ | ✅ | — | ✅ |
| Capacity | ✅ | ✅ | ✅ | ✅ *(staff slots)* | ✅ *(max self-reports)* |
| Registration window | ✅ | ✅ | ✅ | — | ✅ *(self-report window)* |
| Cancellation policy | ✅ | ✅ | ✅ | — | ✅ |
| Eligibility rules | ✅ | ✅ | ✅ | — | ✅ |
| Sessions (multi) | *(deferred)* | ✅ | ✅ | — | — |
| Completion threshold | ✅ *(multi-day)* | ✅ | ✅ | — | ✅ |
...
#### 6.1.2 Type-specific additional fields

- **Task only**: `assigned_to[]` (one or more DSS staff members), `priority` ENUM(`low`, `normal`, `high`). Label "Start date" shown as "Due date" in the form.
- **External only**: `external_organizer` (name of the organizing body, required), `external_url` (event website, optional), `proof_type` ENUM(`certificate`, `attendance_sheet`, `photo`, `other`) — defines what evidence the student must upload for coordinator verification.
- **Volunteering only**: `hours_per_session` (decimal — auto-calculated from session duration if sessions are defined; manually editable). Used to compute total volunteer hours on completion.

#### 6.1.3 Default cancellation policies by type

| Type | Default cancellation policy |
|---|---|
| Event | Anytime until start |
| Program | Deadline 24h before start |
| Volunteering | Deadline 24h before start |
| Task | n/a |
| External | Anytime until start |
...
- **OBEF Community Engagement flag** (optional, default off). Coordinator enables this if the activity should be counted toward the university's OBEF Pillar 6 KPIs. When enabled, two additional fields appear:
  ...
  - **OBEF pre-suggestion by category**: when a category is selected at creation, SAMA pre-suggests the most likely OBEF KPI as a hint (Coordinator can change or dismiss):

    | Category selected | OBEF pre-suggestion |
    |---|---|
    | Awareness / Education | KPI 6.1 — Academic Event |
    | Training & Development | KPI 6.1 — Workshop |
    | Conference / Exhibition | KPI 6.1 — Conference |
    | Community Service | KPI 6.2 — Volunteering |
    | Cultural | KPI 6.2 — Cultural Event |
    | All others | No suggestion |
...
|---|---|---|
| ~~**OT-1**~~ | ~~**Activity sub-types**~~ | ✅ Closed in round 38. See decision #94. |
| ~~**OT-2**~~ | ~~**Activity form fields per type**~~ | ✅ Closed in round 38. See decision #95. | Required vs. optional fields are not fully specified per type. For example: does a Campaign require a capacity? Does a Workshop always need sessions? A field matrix (type × field × required/optional/hidden) needs to be agreed. |
...
| **OT-10** | **DSS Awards module** | The annual DSS Award Ceremony (source: `Awards criteria 2026.pdf`) is entirely unspecified in the PRD. Six awards are defined — Best Club of the Year, Presidential Medal for Student Leadership, Trophy of High Achievement, UD Ambassador Award, Outstanding Athlete Award, and Student Entrepreneur Award. SAMA already holds much of the underlying data (event attendance, volunteer hours, club activities, feedback scores) but the full awards workflow is missing. Key dimensions to specify: (a) **Eligibility auto-screening** — SAMA computes which students/clubs meet the quantitative eligibility thresholds per award (e.g. ≥4 events for Best Club, ≥60 ambassador points for UD Ambassador, ≥5 sport competitions for Outstanding Athlete) using existing activity, attendance, volunteer-hours, and club data; (b) **Endorsement collection** — students endorse other students in-portal; DSS manages the endorsement window and verifies authenticity; (c) **Weighted DSS scoring interface** — Manager scores each shortlisted candidate against the award's weighted criteria rubric (criteria and weights differ per award); (d) **Student vote** — for Best Club of the Year, shortlisted clubs go to a student-wide vote (40% weight); mechanism, voting window, and anti-stuffing rules not defined; (e) **Award history tracking** — Presidential Medal can be received only once per student's academic career; system must block re-nomination; (f) **Dual-award cap** — a student may receive at most 2 awards per cycle from different categories; system enforces this at selection time; (g) **Misconduct cross-reference** — all awards require a clean misconduct record; SAMA currently has no misconduct flag; source system (student conduct/registry) and integration approach not defined; (h) **Sports competition records** — Outstanding Athlete requires participation in ≥5 official sports competitions with coach reports and training logs; these are not currently tracked in SAMA; (i) **External participation points for UD Ambassador** — 1 volunteer hour = 1 point, 1 external conference/competition = 10 points; the volunteer hours part overlaps with OT-5 (volunteer tracking); external conferences are not currently a tracked entity; (j) **Business/startup registration tracking** — Student Entrepreneur Award requires a licensed, actively operating business; no such entity exists in SAMA; IEBC review and recommendation flow is also unspecified; (k) **DSS nomination override** — for Trophy of High Achievement and UD Ambassador, DSS may nominate up to 2 students without full documentation; needs a manager-side override workflow; (l) **Post-award status** — Presidential Medal awardees get an honorary DSS role; UD Ambassador awardees receive a badge; how these are recorded and surfaced in SAMA is not defined; (m) **Award ceremony event** — the ceremony itself is an annual activity that may need a dedicated event type or flag. |
...
| 95 | Round 38 — OT-2 Activity form fields per type resolved. Full field matrix defined in §6.1.1. Key decisions: (1) Start date required for all 5 types (Task = due date); end date/time optional for all; (2) Capacity on all 5 types (Task = staff slots, External = max self-reports); (3) Location required for all except Task; (4) Sessions: Program + Volunteering now; Event multi-session deferred; (5) Certificates: Event, Program, Volunteering only; (6) Budget: all 5 types; (7) Eligibility rules: all except Task; (8) Off-campus toggle: all except Task; (9) OBEF flag: all except Task; (10) Registration window + cancellation policy: all except Task (External = self-report window); (11) Volunteer hours tracking: Volunteering + External; (12) Completion threshold: all except Task; (13) Type-specific fields added: Task gets assigned_to + priority; External gets external_organizer + external_url + proof_type; Volunteering gets hours_per_session. | round 38 |
| 94 | Round 38 — OT-1 Activity sub-types resolved. (1) Activity types updated to 5: Event, Program, Volunteering, Task, External. Workshop and Campaign removed as top-level types — Workshop is covered by Event/Program + Training & Development category; Campaign-style activities are covered by Volunteering type or Event type. (2) Activity categories: 12 system defaults agreed — Sports, Social & Entertainment, Awareness/Education, Community Service, Operations & Admin, Awards & Recognition, Cultural, Training & Development, Academic & Professional Competitions, Conference/Exhibition, Health & Wellness, Orientation & Induction. All fully editable/disableable/renameable by Manager from Settings. (3) Tags: open reusable pool across all categories and types; any coordinator can add a tag on the fly during creation; Manager curates library from Settings. (4) Type-category combinations: fully open, no restrictions enforced. (5) OBEF category pre-suggestion: 5 category-to-OBEF mappings defined as hints (Awareness/Education→6.1, Training & Development→6.1 Workshop, Conference/Exhibition→6.1 Conference, Community Service→6.2 Volunteering, Cultural→6.2 Cultural Event). Coordinator can change or dismiss. (6) Some categories trigger additional form fields (agreed in principle; specific field rules to be defined in OT-2). | round 38 |
